...
try:
    ...
    with conexion:
        ...
        with conexion.cursor() as cursor:
            ...
            # --------------------------------------------------
            # SENTENCIA SQL DELETE MÚLTIPLE
            # Elimina varios registros de la tabla persona
            # utilizando la cláusula IN.
            # --------------------------------------------------
            sentencia = 'DELETE FROM persona WHERE id_persona IN %s'

            # Solicita los IDs de los registros a eliminar
            entrada = input(
                'Digite los números de registros que desee eliminar (separados por coma): '
            )

            # Convierte la entrada del usuario en una tupla
            # compatible con la cláusula IN
            valores = (
                tuple(entrada.split(', ')),
            )
